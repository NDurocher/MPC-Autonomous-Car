...
v_max = 11; v_min = 0;
a_max = 3; a_min = -a_max;
delta_max = deg2rad(25); delta_min = -delta_max;
...
x0 = x;
%2.5; % typical distance from center of gravity of car to front tires
...
%track=load('Crossing/Crossing.csv')';
...
%     args.x0  = x0; % initialization of the optimization problem
...
if(length(yArray)>2)
    csvwrite('Export/Crossing/heading.csv', psi);
    csvwrite('Export/Crossing/velocity.csv', xArray(vstart,:)');
    yArrayHeader = {'Acceleration', 'Steering angle'};
    trackHeader = {'xRoadEnd1', 'yRoadEnd1', 'xRoadEnd2', 'xRoadEnd2', 'xTrack', 'yTrack'};
    savefig('Export/Crossing/Result.fig');
    csvwrite_with_headers('Export/Crossing/Y-Array.csv', yArray', yArrayHeader);
    csvwrite_with_headers('Export/Crossing/Track.csv', track',trackHeader);
end
